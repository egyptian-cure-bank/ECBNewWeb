    ...
    class ToWord
    {
        ...
        private string GetDecimalValue(string decimalPart)
        {
            ...
            if (decimalPart.Length==1)
                result = (Convert.ToInt32(decimalPart) * 10).ToString();
            /*else if (decimalPart.Length == 3 && decimalPart.Substring(0, 1)=="0")
            {
                result = decimalPart.Substring(0, 2);
            }*/
            else if (Currency.PartPrecision != decimalPart.Length)
            {
        ...
        private string ProcessArabicGroup(int groupNumber, int groupLevel, Decimal remainingNumber)
        {
            ...
            if (hundreds > 0)
            {
                if (tens == 0 && hundreds == 2) // حالة المضاف تم التعديل للأفراد والإضافة بناء على طلب البنك
                    retVal = String.Format("{0}", arabicTwos[0]);//retVal = String.Format("{0}", arabicAppendedTwos[0]);
                else //  الحالة العادية
                ...
                { // if we are processing under 20 numbers
                    ...
                    { // This is special case for number 2 when it comes alone in the group
                        /*if (_intergerValue == 2000 || _intergerValue == 2000000 || _intergerValue == 2000000000 || _intergerValue == 2000000000000 || _intergerValue == 2000000000000000 || _intergerValue == 2000000000000000000)
                            ...
                            retVal = String.Format("{0}", arabicTwos[groupLevel]);//  تم التعديل للأفراد والإضافة بناء على طلب البنك - في حالة الافراد
                    }
                    else
                    { // General case
                ...
                { // here we add the new converted group to the previous concatenated text
                    if (group > 0)
                    {
                        ...
                        if (numberToProcess != 2 )
                        {
                            ...
                            else if (numberToProcess % 100 == 1 && group==1)
                                retVal = String.Format("{0} {1}", arabicGroup[group], retVal);
                        }
                    }
                    if (groupDescription == "واحد" && retVal.StartsWith("ألف ") && tempNumber < 1)
